...
const istanbulHtml = readFileSync(join(outputDirectory, 'istanbul-taksi-ucreti', 'index.html'), 'utf8');
const istanbulDescription = "İstanbul'da sarı, turkuaz ve VIP taksi tarifesi farklıdır. 2026 güncel İBB tarifesiyle açılış, km ve zaman ücretini hesaplayın — köprü/tünel geçişi hariç.";
if ((istanbulHtml.match(new RegExp(istanbulDescription.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'))?.length ?? 0) < 3) errors.push('İstanbul: meta, OpenGraph ve Twitter açıklamaları hedef metinle eşleşmiyor.');
if (!/<h1[^>]*>İstanbul Taksi Ücreti Hesaplama 2026<\/h1>/i.test(istanbulHtml)) errors.push('İstanbul: mevcut H1 korunmadı.');
if ((istanbulHtml.match(/İstanbul taksi hesaplama nasıl yapılır\?/g)?.length ?? 0) < 2) errors.push('İstanbul: yeni FAQ görünür içerik ve FAQPage şemasında birlikte bulunmalı.');
if (!istanbulHtml.includes('"@type":"HowTo"') || (istanbulHtml.match(/"@type":"HowToStep"/g)?.length ?? 0) !== 5) errors.push('İstanbul: beş adımlı HowTo şeması eksik.');
if (!istanbulHtml.includes('alt="İstanbul taksi ücreti hesaplama 2026 - güncel sarı taksi tarifesi"')) errors.push('İstanbul: hedef görsel alt metni eksik.');
if (!istanbulHtml.includes('"datePublished":"2026-07-16"') || !istanbulHtml.includes('"dateModified":"2026-07-30"')) errors.push('İstanbul: yayın ve güncelleme tarihleri şemada doğru ayrılmalı.');
if ((istanbulHtml.match(/<time dateTime="2026-07-30">(?:Güncellendi: (?:<!-- -->)?)?30 Temmuz 2026<\/time>/g)?.length ?? 0) < 2) errors.push('İstanbul: görünür güncelleme ve son kontrol tarihleri 30 Temmuz 2026 olmalı.');
if (!istanbulHtml.includes('<dt>Geçerlilik tarihi</dt><dd>16 Şubat 2026</dd>')) errors.push('İstanbul: tarife geçerlilik tarihi korunmadı.');
...
if (!/<a[^>]+href="\/istanbul-taksi-ucreti\/"[^>]*>İstanbul(?:<!-- -->)? taksi ücreti hesaplama<\/a>/i.test(home)) errors.push('Ana sayfa: İstanbul hedef anchor metni eksik.');
if (!/<a[^>]+href="\/istanbul-taksi-ucreti\/"[^>]*>İstanbul taksi ücreti hesaplama<\/a>/i.test(citiesHtml)) errors.push('Şehirler: İstanbul hedef anchor metni eksik.');
const istanbulAirportHtml = readFileSync(join(outputDirectory, 'istanbul-havalimani-taksi-ucreti', 'index.html'), 'utf8');
if (!/<a[^>]+href="\/istanbul-taksi-ucreti\/"[^>]*>İstanbul taksi ücreti hesaplama sayfasına<\/a>/i.test(istanbulAirportHtml)) errors.push('İstanbul Havalimanı: İstanbul şehir sayfasına açıklayıcı anchor eksik.');
