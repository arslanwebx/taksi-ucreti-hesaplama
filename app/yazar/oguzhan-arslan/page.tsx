...
export default function AuthorPage(){
  ...
  return <><JsonLd data={schema}/><ContentShell title={title} description={description} path={path}>
    ...
    <section><h2>Son yazılar</h2><div className="article-grid">{posts.slice(0,9).map((post)=><BlogCard key={post.path} {...post}/>)}</div><p className="author-posts-cta"><Link className="button" href="/blog/">Daha fazla blog yazısı gör</Link></p></section>
  </ContentShell></>;
}
