...
export default function AuthorPage(){
  const schema=[
    {'@context':'https://schema.org','@type':'ProfilePage',name:title,url:canonical(path),mainEntity:{'@type':'Person',name:site.author.name,url:canonical(path),image:canonical(site.author.image),jobTitle:'İçerik yazarı ve tarife araştırmacısı'}},
    {'@context':'https://schema.org','@type':'Person',name:site.author.name,url:canonical(path),image:canonical(site.author.image),jobTitle:'İçerik yazarı ve tarife araştırmacısı'},
  ];
  return <><JsonLd data={schema}/><ContentShell title={title} description={description} path={path}>
    <div className="profile-intro"><img className="author-avatar large" src={site.author.image} alt="Oğuzhan Arslan" width="240" height="240"/><div><h2>Yazar hakkında</h2><p>Oğuzhan Arslan, Taksi Ücreti Hesaplama sitesinin içerik yazarı ve yöneticisidir. Şehir tarifelerini kullanıcıların yolculuk öncesinde anlayabileceği açık bir dille sunar.</p></div></div>
