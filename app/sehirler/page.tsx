  ...
  return (
    <ContentShell title={title} description={description} path="/sehirler/">
      ...
      <section>
        ...
        <div className="article-grid">{cityPosts.map((post) => <BlogCard key={post.path} {...post} title={post.path === '/ankara-taksi-ucreti/' ? 'Ankara taksi ücreti hesaplama' : post.path === '/istanbul-taksi-ucreti/' ? 'İstanbul taksi ücreti hesaplama' : post.title}/>)}</div>
      </section>
      <section>
